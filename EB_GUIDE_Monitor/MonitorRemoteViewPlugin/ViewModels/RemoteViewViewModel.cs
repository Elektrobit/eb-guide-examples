...
namespace MonitorRemoteViewPlugin.ViewModels
{
    ...
    public class RemoteViewViewModel : ViewModel, IDockableComponentViewModel, IMouseClickHandler
    {
        ...
        {
            var isRunning = _continuousScreenshotController.IsRunning;
            if (isRunning)
            {
                await _continuousScreenshotController.StopAsync(SelectedScene);
            }

            try
            {
                var fileSelectionResult = _monitorDialog.SaveFileDialog(
                    DIALOG_WINDOW_TITLE,
                ...
                {
                    _screenshotSavingUtility.SaveScreenshotFromBitmapSource(
                        ScreenshotSource,
                        fileSelectionResult.FileName);
            ...
            finally
            {
                if (isRunning)
                {
                    await _continuousScreenshotController.StartAsync(SelectedScene);
                }
            }
        }
        ...
        {
            await _continuousScreenshotController.ToggleAsync(SelectedScene);
        }
