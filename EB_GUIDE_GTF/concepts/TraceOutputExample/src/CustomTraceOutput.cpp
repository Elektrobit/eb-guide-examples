...
void CustomTraceOutput::configure(const gtf::dependencyresolver::DependencyContainerHandle& container)
{
    ...
    gtf::configuration::StringScalarHandle defaultPathSetting = settings->getItem<gtf::configuration::StringScalar>("gtf.tracing.output.defaultPath");
    if (!defaultPathSetting.valid())
    {
        return;
    }
    ...
    tracePath->setBasePath(defaultPathSetting->get());
