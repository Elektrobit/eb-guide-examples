...
static gtf::dependencyresolver::InterfaceHandle createEventApplication(const gtf::dependencyresolver::DependencyContainerHandle& container)
{
    ...
    if (false == externalInput.valid())
    {
        return defaultApplicationHandle;
    }

    externaltouchinputexample::RunningStageImpl* const runningStage = new externaltouchinputexample::RunningStageImpl(mainDispatcher, externalInput);
