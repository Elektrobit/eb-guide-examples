...
void RunStageImpl::handleEvents(const gtf::eventsystem::global::EventHandle& event)
{
    ...
    if (false == myParameterHandle.valid())
    {
        return;
    }
...
void RunStageImpl::updateWindowList()
{
    ...
    if (false == readIntegerValue(LOCAL_DP_ID_DPWINDOWPOSOUT, windowPosition))
    {
        return;
    }
    ...
    if (false == readIntegerValue(LOCAL_DP_ID_DPWINDOWSIZEOUT, windowSize))
    {
        return;
    }
