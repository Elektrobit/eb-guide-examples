...
MandelbrotImage::MandelbrotImage(uint32_t widthToUse, uint32_t heightToUse, value_type posX, value_type posY, value_type scale, uint32_t maxIterations)
    : width(widthToUse)
    , height(heightToUse)
    , pixelCount(width * height)
{
    ...
    drawMandelbrot(posX - value_type(widthToUse) / scale, posX + value_type(widthToUse) / scale, posY - value_type(heightToUse) / scale, posY + value_type(heightToUse) / scale, maxIterations);
}
...
void MandelbrotImage::drawMandelbrot(value_type cxmin, value_type cxmax, value_type cymin, value_type cymax, uint32_t maxIterations)
{
    ...
    for (uint32_t row = 0; row < imageHeight; ++row)
    {
        ...
        for (uint32_t column = 0; column < imageWidth; ++column)
        {
            ...
            for (iterations = 0; iterations < maxIterations && abs(z) < escapeRadius; ++iterations)
            {
                z = z * z + c;
            }
