...
void RunStageImpl::updateWindowList()
{
    ...
    if (false == readIntegerValue(LOCAL_DP_ID_WINDOWPOSITION, windowPosition))
    {
        return;
    }
    ...
    if (false == readIntegerValue(LOCAL_DP_ID_WINDOWSIZE, windowSize))
    {
        return;
    }
    ...
    if (false == readIntegerValue(LOCAL_DP_ID_WINDOWLISTSIZE, listLength))
    {
        return;
    }
