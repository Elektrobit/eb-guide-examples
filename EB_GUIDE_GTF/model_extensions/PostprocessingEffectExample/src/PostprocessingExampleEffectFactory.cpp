...
void PostprocessingExampleEffectFactory::createEffectPasses(gtf::scdr::EffectPassArray& effectPasses_, gtf::properties::Container* widget_, const gtf::scdr::PostprocessingEffectParams&, gtf::scdr::Allocator& allocator_) const
{
    effectPasses_.resize(EffectPasses::Count);

    // Create Pass
    EffectPass1* pass                  = ALLOC_OBJECT(allocator_, EffectPass1);
...
void PostprocessingExampleEffectFactory::addCustomFramebuffers(gtf::scdr::PostprocessingFramebufferDescriptorArray& framebufferDescriptors_, const gtf::scdr::FramebufferDescriptor&) const
{
    framebufferDescriptors_.resize(0);
}
