...
{
    const uint32_t iWidth  = static_cast<uint32_t>(width);
    const uint32_t iHeight = static_cast<uint32_t>(height);

    const GaussianBlurWeightParams newParams = makeParams(iWidth, iHeight, sigma_);
