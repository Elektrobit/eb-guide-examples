...
public:
    ...
    struct
    {
        gtf::properties::ColorValueHandle   color;
        gtf::properties::FloatValueHandle   xpos;
        gtf::properties::FloatValueHandle   ypos;
        gtf::properties::IntegerValueHandle samples;
        gtf::properties::FloatValueHandle   decay;
        gtf::properties::FloatValueHandle   density;
        gtf::properties::FloatValueHandle   weight;
    } properties;
};
...
CrepuscularRenderer::CrepuscularRenderer(const gtf::scene::TypeResolver& resolver_, gtf::scdr::GraphicsApi driver_)
    ...
    , features()
{
    memset(&propertyRefs, 0, sizeof(propertyRefs));
    memset(&graphicObjects, 0, sizeof(graphicObjects));

    graphicObjects.driver = driver_;

    gtf::scdr::ResolvePropertiesCommon2D(resolver_, CREPUSCULAR_PLUGIN_NAME, propertyRefs.common2d);
    ...
    propertyRefs.color   = wpr[0].property.identifier;
    propertyRefs.xpos    = wpr[1].property.identifier;
    propertyRefs.ypos    = wpr[2].property.identifier;
    propertyRefs.samples = wpr[3].property.identifier;
    propertyRefs.decay   = wpr[4].property.identifier;
    propertyRefs.density = wpr[5].property.identifier;
    propertyRefs.weight  = wpr[6].property.identifier;
}
    ...
    gtf::scdr::Allocator&)
{
    ...
    CrepuscularRenderObject* const object = ALLOC_OBJECT(allocator, CrepuscularRenderObject, graphicObjects, widget_, features.common2d, propertyRefs.common2d);

    if (object)
    {
        gtf::scdr::LoadHandle(widget_, propertyRefs.color, object->properties.color);
        gtf::scdr::LoadHandle(widget_, propertyRefs.decay, object->properties.decay);
        gtf::scdr::LoadHandle(widget_, propertyRefs.density, object->properties.density);
        gtf::scdr::LoadHandle(widget_, propertyRefs.samples, object->properties.samples);
        gtf::scdr::LoadHandle(widget_, propertyRefs.weight, object->properties.weight);
        gtf::scdr::LoadHandle(widget_, propertyRefs.xpos, object->properties.xpos);
        gtf::scdr::LoadHandle(widget_, propertyRefs.ypos, object->properties.ypos);
    }
    return object;
}
    ...
    gtf::scdr::TraverserContext&             traverserContext_)
{
    ...
    gtf::scdr::GetValue(object.properties.color, effectParamsBlock.color);
    gtf::scdr::GetValue(object.properties.decay, effectParamsBlock.decay);
    gtf::scdr::GetValue(object.properties.density, effectParamsBlock.density);
    gtf::scdr::GetValue(object.properties.samples, effectParamsBlock.samples);
    gtf::scdr::GetValue(object.properties.weight, effectParamsBlock.weight);
    gtf::scdr::GetValue(object.properties.xpos, effectParamsBlock.pos[0]);
    gtf::scdr::GetValue(object.properties.ypos, effectParamsBlock.pos[1]);
