...
public:
    ...
    struct
    {
        ...
        gtf::properties::IntegerValueHandle animationTime;
    } properties;
};
...
gtf::scdr::RenderObject* RippleEffect::createRenderObject(gtf::properties::Container& widget_, gtf::scdr::DataStore& dataStore_, gtf::scdr::CommandBuffer&, gtf::scdr::Allocator&)
{
    ...
    RippleEffectRenderObject* object = ALLOC_OBJECT(allocator, RippleEffectRenderObject, widget_, features.common2d, properties.common2d);

    if (object)
    {
        gtf::scdr::LoadHandle(widget_, properties.animationTime, object->properties.animationTime);
        gtf::scdr::LoadHandle(widget_, properties.enabled, object->properties.enabled);
    }
...
RippleEffect::RippleEffect(const gtf::scene::TypeResolver& resolver_)
    : objects(NULL)
{
    gtf::scdr::ResolvePropertiesCommon2D(resolver_, RIPPLEEFFECT, properties.common2d);
    gtf::scdr::ResolveFeaturesCommon2D(resolver_, features.common2d);

    // clang-format off
    gtf::scene::TypeResolver::ContainerPropertyReference wpr[] = 
    {
        { { RIPPLEEFFECT, gtf::properties::Container::Type() }, { "animationTime", gtf::types::limits::uint16_max } },
        { { RIPPLEEFFECT, gtf::properties::Container::Type() }, { "enabled", gtf::types::limits::uint16_max       } }
    };
    // clang-format on
    ...
    gtf::scdr::TraverserContext&             traverserContext_)
{
    ...
    gtf::scdr::GetValue(object.properties.enabled, enabled);
    gtf::scdr::GetValue(object.properties.animationTime, time);
