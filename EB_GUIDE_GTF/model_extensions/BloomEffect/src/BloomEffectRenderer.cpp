...
public:
    ...
    struct
    {
        gtf::properties::ColorValueHandle   luminosityRGB;
        gtf::properties::FloatValueHandle   threshold;
        gtf::properties::FloatValueHandle   luminanceBlurPath;
        gtf::properties::IntegerValueHandle blurIterations;
    } properties;
};
...
{
    gtf::scdr::ResolvePropertiesCommon2D(resolver_, BLOOMEFFECT_PLUGIN_NAME, propertyRefs.common2d);
    ...
    propertyRefs.luminosityRGB     = wpr[0].property.identifier;
    propertyRefs.threshold         = wpr[1].property.identifier;
    propertyRefs.luminanceBlurPath = wpr[2].property.identifier;
    propertyRefs.blurIterations    = wpr[3].property.identifier;
}
    ...
    gtf::scdr::Allocator&)
{
    ...
    BloomEffectRenderObject* const object = ALLOC_OBJECT(allocator, BloomEffectRenderObject, graphicObjects, widget_, features.common2d, propertyRefs.common2d);

    if (object)
    {
        gtf::scdr::LoadHandle(widget_, propertyRefs.blurIterations, object->properties.blurIterations);
        gtf::scdr::LoadHandle(widget_, propertyRefs.luminanceBlurPath, object->properties.luminanceBlurPath);
        gtf::scdr::LoadHandle(widget_, propertyRefs.luminosityRGB, object->properties.luminosityRGB);
        gtf::scdr::LoadHandle(widget_, propertyRefs.threshold, object->properties.threshold);
    }
...
bool BloomEffectRenderer::loadShaderParameters(BloomEffectRenderObject& object_)
{
    bool success = true;

    success &= gtf::scdr::GetValue(object_.properties.luminosityRGB, object_.color);
    success &= gtf::scdr::GetValue(object_.properties.threshold, object_.threshold);
    success &= gtf::scdr::GetValue(object_.properties.luminanceBlurPath, object_.luminanceBlurPath);

    int32_t blurIterations;
    success &= gtf::scdr::GetValue(object_.properties.blurIterations, blurIterations);
    ...
    gtf::scdr::TraverserContext&             traverserContext_)
{
    ...
    if (!loadShaderParameters(object))
    {
        return false;
    }
