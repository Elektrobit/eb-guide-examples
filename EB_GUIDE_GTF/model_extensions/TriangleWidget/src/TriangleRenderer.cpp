...
gtf::scdr::ShaderProgram* TriangleRenderer::createShaderProgram(gtf::scdr::GPUContext& gc_, gtf::scdr::DataStore& dataStore_, gtf::scdr::CommandBuffer& commandBuffer_) const
{
    if ((api != gtf::scdr::GraphicsApi::OpenGLES20) && (api != gtf::scdr::GraphicsApi::OpenGLES3))
    {
        return NULL; // No shaderes available for API
    }

    gtf::scdr::ShaderCache* cache = static_cast<gtf::scdr::ShaderCache*>(dataStore_.queryInterface("gtf::scdr::ShaderCache"));
    ...
    gtf::scdr::ShaderProgram* prog = cache->createProgram(desc, gc_, commandBuffer_);
...
bool TriangleRenderer::init(const gtf::scene::TypeResolver&, gtf::scdr::DataStore& dataStore_, gtf::scdr::CommandBuffer& commandBuffer_, gtf::scdr::Allocator&)
{
    ...
    //
    gtf::scdr::ShaderProgram* const program = createShaderProgram(gc, dataStore_, commandBuffer_);
    if (program)
    {
    ...
    gtf::scdr::ShaderProgram* const programWithAlphaMask = createShaderProgramWithAlphaMask(gc, dataStore_, commandBuffer_);
    if (programWithAlphaMask)
    {
        ...
        // create pipeline state objects and resource binding tables.
        //
        gc.destroyShaderProgram(programWithAlphaMask, commandBuffer_);
    }
    else
    {
        return false;
