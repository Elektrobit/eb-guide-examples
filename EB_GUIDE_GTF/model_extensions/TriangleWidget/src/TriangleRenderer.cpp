...
public:
    ...
    struct
    {
        gtf::properties::ColorValueHandle fillColor;
    } properties;
};

const char* TriangleRenderer::ClassName = TRIANGLE_PLUGIN_NAME;
...
gtf::scdr::ShaderProgram* TriangleRenderer::createShaderProgram(gtf::scdr::DataStore& dataStore_, gtf::scdr::CommandBuffer& commandBuffer_) const
{
    ...
    BIND_SHADER_CACHE(cache, dataStore_);
    gtf::scdr::ProgramDescriptor desc;
    uint32_t                     vertexShaderSize;
    uint32_t                     fragmentShaderSize;
    if (api == gtf::scdr::GraphicsApi::OpenGLES20)
    {
        vertexShaderSize   = static_cast<uint32_t>(strlen(VertexShaderWithAlphaMaskGLES20));
        fragmentShaderSize = static_cast<uint32_t>(strlen(FragmentShaderWithAlphaMaskGLES20));

        desc.vs = gtf::scdr::CShaderDescriptor(&VertexShaderGLES20, &vertexShaderSize);
        desc.ps = gtf::scdr::CShaderDescriptor(&FragmentShaderGLES20, &fragmentShaderSize);
    }
    else if (api == gtf::scdr::GraphicsApi::OpenGLES3)
    {
        vertexShaderSize   = static_cast<uint32_t>(strlen(VertexShaderWithAlphaMaskGLES3));
        fragmentShaderSize = static_cast<uint32_t>(strlen(FragmentShaderWithAlphaMaskGLES3));

        desc.vs = gtf::scdr::CShaderDescriptor(&VertexShaderGLES3, &vertexShaderSize);
        desc.ps = gtf::scdr::CShaderDescriptor(&FragmentShaderGLES3, &fragmentShaderSize);
    }
    ...
    gtf::scdr::CommandBuffer& commandBuffer_) const
{
    ...
    if (api == gtf::scdr::GraphicsApi::OpenGLES20)
    {
        ...
        desc.vs = gtf::scdr::CShaderDescriptor(&VertexShaderWithAlphaMaskGLES20, &vertexShaderSize);
        desc.ps = gtf::scdr::CShaderDescriptor(&FragmentShaderWithAlphaMaskGLES20, &fragmentShaderSize);
    }
    else if (api == gtf::scdr::GraphicsApi::OpenGLES3)
    {
        vertexShaderSize   = static_cast<uint32_t>(strlen(VertexShaderWithAlphaMaskGLES3));
        ...
        desc.vs = gtf::scdr::CShaderDescriptor(&VertexShaderWithAlphaMaskGLES3, &vertexShaderSize);
        desc.ps = gtf::scdr::CShaderDescriptor(&FragmentShaderWithAlphaMaskGLES3, &fragmentShaderSize);
    }
...
gtf::scdr::Color getFillColor(const TriangleRenderObject& object_)
{
    gtf::scdr::Color fillColor;
    gtf::scdr::GetValue(object_.properties.fillColor, fillColor);
    return fillColor;
}
    ...
    gtf::scdr::TraverserContext&             traverserContext_)
{
    ...
    object.triangleParams.fillColor     = getFillColor(object) * object.coloration;
    ...
    gtf::scdr::Allocator&)
{
    ...
    TriangleRenderObject* object = ALLOC_OBJECT(allocator, TriangleRenderObject, widget_, features, properties.common2d);

    if (object)
    {
        gtf::scdr::LoadHandle(widget_, properties.fillColor, object->properties.fillColor);
    }
