...
static gtf::dependencyresolver::InterfaceHandle createPreparedStage(const gtf::dependencyresolver::DependencyContainerHandle& container_)
{
	if (!container_)
	{
	...
	return gtf::dependencyresolver::InterfaceHandle(new labelshapewidgetfeature::PreparedStageImpl(container_));
}
...
GTF_PLUGIN_SO_SYMBOL void GTF_PLUGIN_EXTEND_CATALOG(void * catalog)
{
	...
	gtf::dependencyresolver::extendCatalog<gtf::scdr::DynamicFeatureCollection, createGenericWithResolver<labelshapewidgetfeature::ExtendedDynamicFeatureCollection>>(dependencyCatalog);
