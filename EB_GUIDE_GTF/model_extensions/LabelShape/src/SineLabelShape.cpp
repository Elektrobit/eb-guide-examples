    ...
    , stepWidth(0.f)
{
    ...
    isAssigned = model->getFeatures().has(fpr[0].featureType.identifier);

    if (isAssigned)
    {
        scdr::LoadFeatureHandle(*model, fpr[0], sineCurveFeatures.enabled);
        scdr::LoadFeatureHandle(*model, fpr[1], sineCurveFeatures.amplitude);
        scdr::LoadFeatureHandle(*model, fpr[2], sineCurveFeatures.periodLength);
        scdr::LoadFeatureHandle(*model, fpr[3], sineCurveFeatures.horizontalOffset);
        scdr::LoadFeatureHandle(*model, fpr[4], sineCurveFeatures.verticalOffset);
        scdr::LoadFeatureHandle(*model, fpr[5], sineCurveFeatures.cycleCount);
    }
}
...
bool labelshapewidgetfeature::SineLabelShape::updateModel()
{
    if (isAssigned)
    {
        ...
        scdr::GetValue(sineCurveFeatures.enabled, sineCurveFeatureValues.enabled);

        const float amplitude = sineCurveFeatureValues.amplitude;
        scdr::GetValue(sineCurveFeatures.amplitude, sineCurveFeatureValues.amplitude);
        ...
        const float periodLength = sineCurveFeatureValues.periodLength;
        scdr::GetValue(sineCurveFeatures.periodLength, sineCurveFeatureValues.periodLength);
        ...
        scdr::GetValue(sineCurveFeatures.horizontalOffset, sineCurveFeatureValues.horizontalOffset);
        ...
        scdr::GetValue(sineCurveFeatures.verticalOffset, sineCurveFeatureValues.verticalOffset);
        ...
        scdr::GetValue(sineCurveFeatures.cycleCount, sineCurveFeatureValues.cycleCount);
