...
extendedcontainerwidget::RenderObject::RenderObject(const gtf::properties::Container& widget_, const gtf::scdr::WidgetFeaturesCommon2D& featureHandles_)
{
    //
    // We need to map the feature handles to actual feature pointers
    //
    gtf::scdr::GetWidgetFeaturesCommon2DModels(widget_, featureHandles_, features);
}
...
{
    if (features.touch && width > 0.f && height > 0.f)
    {
        return this;
    }
    return NULL;
